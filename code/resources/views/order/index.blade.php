              ...
              <div class="p-6 bg-white border-b border-gray-200 flex flex-col">
                  <div class="w-full flex flex-row-reverse mb-2.5">
                      ...
                      <button
                        disabled
                        id="button-mult" 
                        onclick="event.preventDefault(); if(confirm('Tem certeza que deseja deletar os pedidos selecionados?')) { document.getElementById('destroy-form-mult').submit(); }"
                        class="mx-1 bg-red-400 hover:bg-red-600 px-3 py-1 rounded text-white focus:outline-none"
                        >Deletar Selecionados
                      </button>
                      <form id="destroy-form-mult" action="{{ route('order.multDestroy') }}" method="POST" style="display: none;">
                        @csrf
                        @method('DELETE')
                      </form>    
                  </div>
                      ...
                      <tbody>
                        @forelse ($orders as $order)
                          <tr class="{{ $loop->index % 2 != 0 ? 'bg-gray-100' : '' }}">
                              ...
                              <td class="text-center">
                                <input value="{{ $order->id }}" m-type="order" class="checkbox" type="checkbox">  
                              </td>
                          </tr>
                        @empty
                          <tr>
                              <td class="text-center" colspan="6">Sem Pedidos cadastrados</td>
                          </tr>
                        @endforelse
                      </tbody>
                    </table>
