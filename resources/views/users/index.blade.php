                ...
                <div class="card-body">
                                ...
                                <div class="row form-group mb-2">
                                    <div class="col-12 col-sm-4 pr-sm-2">
                                        <select class="form-control mr-2 mb-2" name="orderby">
                                            ...
                                            <option value="email" {{ ($page['order'][0] == 'email') ? 'selected' : '' }}>E-mail</option>
                                            <option value="document" {{ ($page['order'][0] == 'document') ? 'selected' : '' }}>CPF</option>
                    ...
                    <form action="{{ route('users.mass-destroy') }}" method="post" id="mass-destroy">
                        ...
                        <div class="row mb-2 justify-content-end">
                            <div class="col-12 col-sm-4 col-md-3 col-xl-2 mb-2 pr-sm-1">
                                <button type="submit" class="btn btn-danger btn-block">Excluir usuários</a>
                            </div>
                            <div class="col-12 col-sm-4 col-md-3 col-xl-2 mb-2 pl-sm-1">
                                <a href="{{ route('users.create') }}" class="btn btn-primary btn-block">Registrar usuário</a>
                            </div>
                        </div>
                    </form>
                    ...
                    <ul class="pagination justify-content-end">
                        <li class="page-item {{ $pagination['current'] == 1 ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $pagination['current'] == 1 ? '#' : route('users.index') . '?page=1&items=' . $page['items'] }}">
                                <i class="fas fa-angle-double-left"></i>
                            </a>
                        </li>
                        <li class="page-item {{ $pagination['current'] == 1 ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $pagination['current'] == 1 ? '#' : route('users.index') . '?page=' . ($pagination['current'] - 1). '&items=' . $page['items'] }}">
                        ...
                        @for ($i = $start; $i <= $end; ++$i)
                            ...
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ route('users.index') . '?page=' . $i . '&items=' . $page['items'] }}">{{ $i }}</a>
                                </li>
                            @endif
                        @endfor

                        <li class="page-item {{ $pagination['current'] == $pagination['total'] ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $pagination['current'] == $pagination['total'] ? '#' : route('users.index') . '?page=' . ($pagination['current'] + 1) . '&items=' . $page['items'] }}">
                                <i class="fas fa-chevron-right"></i>
                            </a>
                        </li>
                        <li class="page-item {{ $pagination['current'] == $pagination['total'] ? 'disabled' : '' }}">
                            <a class="page-link" href="{{ $pagination['current'] == $pagination['total'] ? '#' : route('users.index') . '?page=' . $pagination['total'] . '&items=' . $page['items'] }}">
