...
@section('content')
    <div class="antialiased sans-serif bg-gray-200">
        <div class="container mx-auto py-6 px-4">
            <h1 class="text-3xl py-4 border-b mb-10">CRUD de Produtos</h1>
            ...
            <a href="{{ route('product.create') }}"
                class="bg-gray-800 inline-flex mt-12 text-white rounded-full p-2 hover:bg-gray-700">Cadastrar novo
                produto</a>
                    ...
                    <table class="min-w-full leading-normal">
                        ...
                        <tbody>
                            @if (isset($productsResults))
                                @foreach ($productsResults as $filter)
                                <tr>
                                    ...
                                    <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                        <a href="{{ route('orders.show', $filter->id) }}"
                                            class="text-grey-lighter font-bold py-1 px-3 rounded text-xs bg-blue-400 hover:bg-blue-500">Ver</a>
                                        <a href="{{ route('product.edit', $filter->id) }}"
                                        ...
                                        <a href="javascript:if(confirm('Deseja realmente excluir?')){
                                                        window.location.href ='{{ route('product.destroy', $filter->id) }}'
                                                    }"
                            ...
                            @else
                                @foreach ($products as $product)
                                    <tr>
                                        ...
                                        <td class="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                                            <a href="{{ route('orders.show', $product->id) }}"
                                                class="text-grey-lighter font-bold py-1 px-3 rounded text-xs bg-blue-400 hover:bg-blue-500">Ver</a>
                                            <a href="{{ route('product.edit', $product->id) }}"
                                                class="text-grey-lighter font-bold py-1 px-3 rounded text-xs bg-green-400 hover:bg-green-500">Editar</a>
                                            <a href="javascript:if(confirm('Deseja realmente excluir?')){
                                                            window.location.href ='{{ route('product.destroy', $product->id) }}'
                                                        }"
                                                class="text-grey-lighter font-bold py-1 px-3 rounded text-xs bg-red-400 hover:bg-red-500">Excluir</a>
                                        </td>
                                        </td>
                    ...
                    @if (isset($productsResults))
                        <div
                            ...
                            <span class="text-xs xs:text-sm text-gray-900">
                                Mostrando {{ $productsResults->count() }} de
                                {{ $productsResults->total() }} resultados
                            </span>
                            <div class="inline-flex mt-2 xs:mt-0">
                                @if ($productsResults->previousPageUrl() != null)
                                    <a href="{{ $productsResults->appends(Request::all())->previousPageUrl() }}"
                                ...
                                @endif
                                @if ($productsResults->nextPageUrl() != null)
                                    <a href="{{ $productsResults->appends(Request::all())->nextPageUrl() }}"
                                        class="text-sm bg-gray-300 hover:bg-gray-400 text-gray-800 font-semibold py-2 px-4 rounded-r">
                                        Próxima
                                    </a>
                                @else
