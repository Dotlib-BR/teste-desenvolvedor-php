...
export default {
  async mounted() {
    const id = localStorage.getItem('cliente-edit-id')
    if (!!id) {
      this.id = id
      const { data } = await axios.get(`/api/clientes/${id}`)
      this.form.CPF = data.result.CPF
      this.form.Nome = data.result.Nome
      this.form.Email = data.result.Email
    } else {
      this.$router.go(-1)
    }
    localStorage.removeItem('cliente-edit-id')
  },
  ...
  methods: {
    ...
    async submitForm() {
      ...
      const { data } = await axios.put(`/api/clientes/${this.id}`, this.form)
