    ...
    <tr>
        ...
        <td>
            @foreach ($vaga->candidatos as $candidato)
                @if ($candidato->id === Auth::user()->id && $candidato->pivot->vaga_id === $vaga->id)
                    <button class="btn btn-success">Candidatado!</button>
                    @break
                @endif
            @endforeach

            @if (!$vaga->candidatos->contains(Auth::user()->id))
            <a href="{{ route('detalhes-vaga', ['vaga' => $vaga->id]) }}" class="btn btn-primary">Candidatar-se à Vaga</a>
